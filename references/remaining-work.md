...
- Integrated the v1.5.2 `spec-board` surface into the current v2.1 wrapper without downgrading existing reliability/governance features.
- `macro spec-board open|show|ensure|path|check|uncheck|ladder|markdown|clear`.
- `app-profile` now includes `spec_board` for PLC/SCADA-like windows.
- Added nested `xg5000-ladder-diagnostician` skill and deterministic `diagnose-ladder.ps1`.

Current progress estimate: **100%** (v2.1.1). Helper persistent server, wrapper daemon,
