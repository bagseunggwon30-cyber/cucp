...
## 9. xg5000-evidence 호환성

`macro list-affordances` 의 top-level 응답은 `affordance_count`, `from_cache`, `affordances` 같은 legacy 필드를 그대로 유지합니다. 새 `_envelope` (cucp.observation/v1) 는 추가 필드일 뿐이며, 기존 xg5000-evidence parser 는 영향받지 않습니다.
