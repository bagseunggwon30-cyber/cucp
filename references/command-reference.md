...
### spec-board (XG5000 project spec + kanban board)

`spec-board` opens a compact always-on-top Kanban board for XG5000 work. It stores the PLC environment, network settings, device map, checklist cards, warnings, and a ladder draft. The board is intentionally passive: it has no screenshot loop or polling, so it does not add OCR/vision load while XG5000 is running.

```powershell
& <wrapper> macro spec-board open
& <wrapper> macro spec-board show
& <wrapper> macro spec-board check --id XG-001
& <wrapper> macro spec-board uncheck --id XG-001
& <wrapper> macro spec-board ladder
& <wrapper> macro spec-board markdown
& <wrapper> macro spec-board path
```

The JSON file is stored at `%TEMP%\computer-use-control-plane\spec-board\current-spec-board.json` and uses schema `cucp.spec-board/v1`. `app-profile` automatically includes this JSON as `spec_board` for PLC/SCADA-like windows, so planning can follow the board's CPU model, Ethernet/XGT Dedicated settings, device map, task checklist, and safety warnings.
...
- `spec_board`: the current `cucp.spec-board/v1` JSON. PLC/SCADA-like apps auto-create/load it so environment,
  network, device map, checklist, ladder notes, and warnings can guide the plan.
